...
class PydanticAiConfig:
    """Simple loader for PydanticAI configuration."""

    def __init__(self, config_path: str = "pydantic_ai.json"):
    ...
    def _load_agents(self) -> dict[str, str]:
        ...
        if not self.exists:
            raise FileNotFoundError(
                f"PydanticAI configuration file not found at {self.config_path}"
            )
        ...
        try:
            with open(self.config_path, "r") as f:
                ...
                if "agents" not in config:
                    raise ValueError(
                        "Missing 'agents' key in pydantic_ai.json configuration file."
                    )
