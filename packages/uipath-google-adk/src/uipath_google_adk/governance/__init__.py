...
Registers :class:`GoogleADKAdapter` with the global adapter registry in
``uipath.core.adapters`` so ``uipath.runtime.governance.GovernanceRuntime``
can attach the ADK-specific inner hooks (BEFORE_MODEL, AFTER_MODEL,
TOOL_CALL, AFTER_TOOL) when it sees a Google ADK agent.
...
Wiring:
    1. Importing this module triggers registration as a side-effect, so
       any caller that does ``import uipath_google_adk.governance`` is
       opted in.
    2. The package also exposes :func:`register_governance_adapter` as an
       entry point under ``uipath.governance.adapters`` so the registry's
       entry-point discovery can plug us in without an explicit import.
"""
