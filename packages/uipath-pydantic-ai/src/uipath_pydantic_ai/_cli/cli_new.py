...
def generate_script(target_directory):
    ...
    template_pydantic_ai_json_path = os.path.join(
        os.path.dirname(__file__), "_templates/pydantic_ai.json.template"
    )
    target_path = os.path.join(target_directory, "pydantic_ai.json")
    shutil.copyfile(template_pydantic_ai_json_path, target_path)
...
dependencies = [
    "uipath-pydantic-ai>=0.0.1, <0.1.0",
]
...
def pydantic_ai_new_middleware(name: str) -> MiddlewareResult:
    """Middleware to create demo PydanticAI agent"""
    ...
    try:
        with console.spinner(f"Creating new agent {name} in current directory ..."):
            ...
            console.success("Created 'pydantic_ai.json' file.")
